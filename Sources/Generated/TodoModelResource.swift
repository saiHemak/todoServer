...
public class TodoModelResource {
    private let adapter: TodoModelAdapter
    private let path = "/api/todoModels"
    private let pathWithID = "/api/todoModels/:id"

    init(factory: AdapterFactory) throws {
        adapter = try factory.getTodoModelAdapter()
    }
    ...
    private func handleCreate(request: RouterRequest, response: RouterResponse, next: @escaping () -> Void) {
        ...
        do {
            let model = try TodoModel(json: json)
