...
public class TodoModelMemoryAdapter: TodoModelAdapter {
    var items: [String:TodoModel] = [:]

    public func findAll(onCompletion: @escaping ([TodoModel], Error?) -> Void) {
        onCompletion(items.map { $1 }, nil)
    }

    public func create(_ model: TodoModel, onCompletion: @escaping (TodoModel?, Error?) -> Void) {
    ...
    public func findOne(_ maybeID: String?, onCompletion: @escaping (TodoModel?, Error?) -> Void) {
    ...
    public func update(_ maybeID: String?, with model: TodoModel, onCompletion: @escaping (TodoModel?, Error?) -> Void) {
    ...
    public func delete(_ maybeID: String?, onCompletion: @escaping (TodoModel?, Error?) -> Void) {
        guard let id = maybeID else {
            return onCompletion(nil, AdapterError.invalidId(maybeID))
        }
